...
**2023：Cursor 崛起，垂直产品哲学确立。** 在 AutoGPT 等框架级 agent 集体失效于生产的背景下，Cursor 用完全不同的路线突破：不去解决"完整任务自主执行"，而是把模型不确定性包裹在 IDE UI 里（diff view、inline accept/reject、context picker）。核心赌注是"模型还不够可靠，所以 UI 要做缓冲"。这个哲学让它成为第二代第一个真正有付费用户支撑的 coding agent 产品，也定义了"developer-in-the-loop"这个产品范畴。

**2024-03：Devin 发布（Cognition AI），确立新产品范畴。** 首次宣称"AI 软件工程师"而非"AI 编程助手"，在 SWE-bench 上达到 13.86%——在当时是革命性的数字（此前没有模型/系统超过 5%）。初始定价 $500/月，定位企业级。更深层的意义在于：Devin 把 coding agent 的产品定义从"帮你写代码的工具"升级为"可以被分配任务的 agent"——接受一个 GitHub issue，自主完成，推出 PR。这个产品定义随后主导了整个行业的想象，SWE-bench 也从此成为 coding agent 的军备竞赛标尺。
...
- 2026-05-03：补充 Cursor 崛起（2023）和 Devin 发布（2024-03）两个关键产品事件
- 2026-04-27：初次创建
