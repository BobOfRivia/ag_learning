...
**2025-04：Devin 2.0 + 定价塌缩（$500→$20/月）。** 能力快速商品化的最清晰信号。Devin 2.0 引入完整 IDE 界面（VS Code 风格），面向个人开发者开放。定价从企业级 $500/月降到 $20/月，降幅 96%。标志着 coding agent 竞争重心从"能不能做到"转向"怎么定价、怎么获客"。同月 ARR 开始爆发，6 月达到 $73M（从 2024-09 的 $1M 起步，9 个月 73 倍增长）。

**2025-04：Codeium 品牌重塑为 Windsurf。** Cascade（Windsurf 的核心 agent）能理解整个 codebase、多文件编辑、执行 terminal 命令，设计上比 Cursor 更向"自主执行"倾斜，比 Devin 更保留"developer-in-the-loop"。这个产品形态事后被证明是市场真正需要的混合形态。

**2025 年：企业轨道独立成型。** 与开发者工具轨道并行，企业自动化轨道在这一年密集落地：Salesforce Agentforce 迭代至 3.0（6 月），完成从 CRM 嵌入式 agent 到跨工作流自主执行的升级；Glean 推出 Glean Agents（2025-02），将企业知识库搜索扩展为可操作的工作流 agent；Harvey（法律 AI）完成 $300M 融资，估值 $3B（2025-02），成为垂直 AI agent 标杆。这三者的共同特征是：面向业务角色而非工程师、嵌入已有工作流而非独立工具、用业务 KPI 而非 SWE-bench 评估效果。

**2025-11：OpenHands CodeAct 2.1 + SWE-bench 72%。** 使用 Claude Sonnet 4.5（extended thinking），OpenHands 达到 SWE-bench Verified 72% 解决率——从 Devin 2024-03 的 13.86% 基准出发，18 个月内提升约 5 倍。SWE-bench Verified 开始失去区分度，SWE-EVO 随后出现作为更难替代（最强模型 GPT-5 仅解决 19-21%）。
...
**2025-12：Cognition 收购 Windsurf（约 $250M）。** Coding agent 领域最大 M&A。Cognition 将 Devin 的自主执行能力与 Windsurf/Cascade 的 IDE 交互体验合并，形成混合产品——收购的逻辑本身就是市场判断：纯自主（Devin）和纯辅助（Cursor 式）都不是终态，hybrid 才是。收购时 Windsurf 已有 $82M ARR、350+ 企业客户。

**2026-02：Hermes Agent（Nous Research）——第四代早期信号。** 持久跨 session 记忆（FTS5 + LLM summarization）+ 从执行经验自主生成可复用 skill，运行在 $5 VPS 上永久驻留。第一个认真实现"agent 作为 resident 而非被调用工具"范式的产品。其核心技术赌注——随时间积累能力而非每次重置——如果成立，将对当前所有工具型 agent 的商业模式构成挑战。详见 →（topics/agent-product-landscape.md）
...
第三代的产品格局沿两条独立轨道演进，详细分析见 → [topics/agent-product-landscape.md](../topics/agent-product-landscape.md)。这里只标注与工程演进直接相关的结构性观察。

**开发者工具轨道**形成了明确的哲学分叉：以 Cursor 为代表的"UI scaffolding 派"（模型不完全可信，UI 做缓冲）vs 以 Claude Code 为代表的"模型信任派"（推理模型已够可靠，直接 CLI 授权）。Cognition 收购 Windsurf 后形成的混合产品，暗示市场真正需要的是这两个极端之间的某个位置。

**企业自动化轨道**（Salesforce Agentforce、Glean、Harvey）在工程上代表不同的选择：垂直深度（Harvey = 法律领域 + o1 推理）vs 水平广度（Glean = 企业全知识图谱）。两条路线目前都没有被证伪。

**SWE-bench 竞赛**是第三代最重要的产品驱动力之一。数据轨迹：2024-03（Devin, 13.86%）→ 2025-11（OpenHands + Claude Sonnet 4.5, 72%）→ SWE-EVO 出现（GPT-5 仅 19-21%）。这个军备竞赛既推动了真实能力提升，也扭曲了产品优先级——工程资源被拉向"让 benchmark 好看"而非"解决真实用户场景"。
...
- 2026-05-03：补充产品竞争格局章节；在关键事件中增加 Devin 2.0 定价塌缩、Windsurf 品牌重塑、企业轨道落地、SWE-bench 数据轨迹、Cognition 收购 Windsurf、Hermes Agent 早期信号等条目；关联 topics/agent-product-landscape.md
