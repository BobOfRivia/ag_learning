...
许多 agent 任务名义上是"动作"（删除文件、迁移代码、生成清单），但内部隐含一个未被显式输出的"判定"阶段——该删什么、如何迁移、以什么角度生成。当判定隐藏在动作里时，错误会无声地传递到执行：用户看到的是动作的结果，但无法在动作完成前审视判定本身。

来自另一次代码清理会话的观察案例。任务是删除"已迁移到数据层"的代码。agent 第一次回应直接基于 CLAUDE.md 给出了删除清单——这一步看起来是个简单动作（"列出待删文件"），但隐藏了一个被跳过的判定（"每一项是否真的不再被引用"）。结果是清单里包含了实际仍被 consumer 复用的 service。修正后的工作方式是把判定显式抽出作为单独一轮输出："watch_service / port_service 仅被待删 router 用 → 可删；im_clean_service 被 tariff_extract_consumer:148 复用 → 必须保留"——动作在判定被用户认可后才执行。

这映射到 context engineering 的视角是：**判定输出本身就是一种 Write 操作**——把 agent 内部隐性的推理外化为 context 中可观察、可纠正的内容。隐性推理在 compaction 时会被丢弃（参见后文"中间推理过程丢失"），显性判定则作为可见状态留在会话历史里。当后续步骤需要回溯时，显性判定提供了"为什么这样做"的证据；隐性推理只能重新执行。

结构性原则：**对不可逆动作（删除、迁移、发布、写入跨 session 的持久层），强制 agent 在执行前单独输出一轮判定并标注依据；对可逆动作（读、查询、本地编辑）允许判定与动作合并**。这是按"代价"分级的主动性策略，而非统一的 verbosity 选择。判定阶段确实在 context 中占据了额外空间，但它换来一个不可替代的副作用：**用户在不可逆动作启动前获得审计点**。
...
- 2026-05-07：在"理论之外的实践技术"中新增"判定与动作的显式拆分"小节，案例来自一次代码清理会话——agent 第一次回应跳过判定阶段直接给出删除清单导致错误，修正后把判定作为独立一轮显式输出
