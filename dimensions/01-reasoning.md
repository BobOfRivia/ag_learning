...
- **× Instruction Following（维度 5）**：Reasoning model 在指令遵循上有双面效应——能"想一想"再执行复杂约束，但 thinking 过程也可能"想偏"而偏离原始指令。Instruction gap 数据证实 reasoning model 在 IF 上不比通用模型更好（DeepSeek-R1：1,061 违规 vs GPT-5：660 违规）。详见 [dimensions/05-instruction-following.md](05-instruction-following.md)
