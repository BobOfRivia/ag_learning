...
**× Instruction Following（维度 5）**：Tool use 的可靠性高度依赖模型遵循 schema 约束的能力。strict mode 本质上是在推理时强制提升 instruction following。工具描述中的约束条件（参数范围、调用前提）需要模型精确遵循。详见 [dimensions/05-instruction-following.md](05-instruction-following.md)
