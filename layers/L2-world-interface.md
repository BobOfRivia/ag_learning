...
**进阶 RAG 变体的成熟度分层。** 2026 年的 RAG 实践已从"做不做 RAG"演进到"做哪一层 RAG"——四个成熟度层级（Naive / Advanced / Modular / Agentic，以及作为 agentic 最佳实践的 Adaptive RAG）有清晰的演进关系和适用边界。生产主流停留在 Advanced RAG（带混合检索 + 交叉编码器重排序），持续迭代检索质量的团队走 Modular，复杂多跳场景走 Agentic（通常以 Adaptive 形态出现以控制成本）。一个被反复验证的生产发现：**RAG 失败时约 73% 的根因在检索环节而非生成环节**——这把工程投资优先级钉死在检索质量（混合搜索、重排序、分块策略）而非生成端 prompt 优化。

四层架构的内部机制、各层的落地程度判断、以及混合检索 / HyDE / 交叉编码器重排序 / 语义分块等关键生产模式的展开，详见 [topics/advanced-rag-variants.md](../topics/advanced-rag-variants.md)。
...
注：四层架构（Naive / Advanced / Modular / Agentic / Adaptive）和关键生产模式的详细参考资料在 [topics/advanced-rag-variants.md](../topics/advanced-rag-variants.md) 的信息源章节。
...
- 2026-05-05：将 RAG 四层架构分类（Naive/Advanced/Modular/Agentic/Adaptive）和关键生产模式（混合检索、重排序、查询变换、语义分块、73% 检索失败定律）拆分到 topics/advanced-rag-variants.md。L2 保留 RAG 整体生态判断和分层协作判断，详细机制和参考资料移入 topic
