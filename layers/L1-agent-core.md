...
Agent loop 是原子操作，编排模式是如何组合这些原子操作来完成复杂任务。Anthropic 在 2024 年末的 "Building Effective Agents" 中提出了一个被广泛引用的分类框架，核心区分是 **workflow**（LLM 和工具通过预定义的代码路径编排）和 **agent**（LLM 动态控制自身的执行流程和工具使用）。Workflow 用可预测性换灵活性，agent 用灵活性换可预测性。这不只是技术选型，而是对**控制权归属**的根本判断——控制权留在代码手里，还是交给模型？Workflow 信任模型做局部决策（生成文本、分类输入），整体路径由代码控制；agent 信任模型做全局规划，包括决定使用什么工具、走什么路径。Anthropic 的立场是在当前模型能力下应渐进式扩展这种信任：从最小可行系统开始，只在验证可靠后才向 agent 方向推进——**"找到最简单可行的解决方案，只在必要时才增加复杂度"，这甚至意味着完全不构建 agent 系统。**
...
**Workflow vs Agent 的选型信号。** 选 workflow 的信号：任务步骤可以在设计时枚举；每步结果需要独立审计；错误代价高且不可逆；需要稳定的延迟和可预期的成本。选 agent 的信号：任务路径在运行前无法预知（如复杂代码重构、多源信息研究）；需要根据中间结果动态调整策略；灵活性的价值高于可控性的成本。实践中有一个系统性偏差值得注意：agent 工程社区普遍倾向高估 agent 的必要性，很多场景用结构化 workflow + 好的 context 就能解决，但因为"agent"更诱人而被过度架构。
...
- 2026-05-04：在「编排模式谱系」中补充 workflow vs agent 的控制权归属框架、选型信号和「最简可行」设计原则
